-- Founder-adopted Remodel Studio pass: USD 19 one-time, up to three concepts.
-- This migration closes the earlier four-concept architecture without opening
-- generation or changing any payment/deposit gate.

alter table public.remodel_concepts
  drop constraint if exists remodel_concepts_ordinal_check;

alter table public.remodel_concepts
  add constraint remodel_concepts_ordinal_check check (ordinal between 1 and 3);

create index if not exists studio_entitlements_owner_idx
  on public.studio_entitlements(owner_user_id);
...
begin
  ...
  if concept_count >= 3 or p_ordinal > 3 or exists (
